const PROJECTS_DATA = [
    {
        id: "sudoku",
        title: "Jeu de Sudoku",
        description: "Implémentation complète d'un jeu de Sudoku en langage C, avec gestion de l'interface et des règles.",
        image: "./img/projects/sudoku/sudoku_generic.jpg",
        icon: "fa-solid fa-puzzle-piece",
        tags: ["C", "Algorithmique"],
        github: "https://github.com/Piel-M/sudoku_in_c",
        delay: 0
    },
    {
        id: "sudoku-resolver",
        title: "Résolveur Sudoku",
        description: "Algorithme de backtracking optimisé capable de résoudre des grilles de n'importe quelle taille en quelques millisecondes.",
        image: "./img/projects/sudoku_res/sudoku_code.png",
        icon: "fa-solid fa-robot",
        tags: ["C", "Backtracking"],
        github: "https://github.com/Piel-M/sudoku_resolver_in_C",
        delay: 100
    },
    {
        id: "tabaskho",
        title: "Tabas'kho",
        description: "Site web vitrine responsive pour une plateforme de streaming fictive, axé sur l'UX/UI.",
        image: "./img/projects/tabaskho/thabasko1.png",
        icon: "fa-solid fa-globe",
        tags: ["HTML/CSS", "Design"],
        github: "https://github.com/Piel-M/tabaskho",
        delay: 200
    },
    {
        id: "documentation",
        title: "Générateur de Doc",
        description: "Outil automatisé pour générer une documentation technique propre à partir de commentaires dans le code source C.",
        image: "./img/projects/documentation/documentation.png",
        icon: "fa-solid fa-file-code",
        tags: ["Java/C", "Regex"],
        github: "https://github.com/Piel-M/technical_documentation_for_c_files",
        delay: 0
    },
    {
        id: "bdd",
        title: "Architecture BDD",
        description: "Conception et implémentation d'une base de données complexe, modélisation UML et requêtes SQL optimisées.",
        image: "./img/projects/bdd/class-diagram.png",
        icon: "fa-solid fa-database",
        tags: ["SQL", "UML"],
        github: null,
        delay: 100
    },
    {
        id: "gestion-projet",
        title: "Gestion de Projet",
        description: "Pilotage d'un projet IT en équipe : méthode Agile, matrice RACI et suivi des livrables.",
        image: "./img/projects/gestion_projet/RACI.png",
        icon: "fa-solid fa-users-gear",
        tags: ["Agile", "Management"],
        github: null,
        delay: 200
    }
];
